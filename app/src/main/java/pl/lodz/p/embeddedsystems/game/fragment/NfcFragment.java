...
public class NfcFragment extends Fragment implements PropertyChangeListener {
  ...
  /** Wstępnie inicjuje fragment danymi. Tworzenie adaptera nfc. */
  @Override
  ...
  /**
   * Obserwowanie zdarzeń w aplikacji. Przy tworzeniu ustawia przechwytywanie NFC. Otrzymuje
   * informacje gdy zostanie wykryty tag.
   *
  ...
  private void handleTag(@NonNull Tag tag) {
    ...
    if (Arrays.equals(tag.getId(), cheatTag)) {
      ...
      Toast.makeText(
              this.getContext(),
              (isCheatMode
                  ? "[NFC] Tryb oszusta został włączony."
                  : "[NFC] Tryb oszusta został wyłączony."),
              Toast.LENGTH_LONG)
          .show();
    } else {
