...
//  private static final int SCORING_ZONE_LIMIT = 100;
  ...
  public static boolean checkRules(
    ...
    if (isInRange(scoringZone, playerBall)) {
      ...
      if (randomDouble < probability) {
        scoringZone.setRadius((float) ThreadLocalRandom.current().nextDouble(100f, 300f));
        float offset = scoringZone.getRadius();
        scoringZone.setCenter(
            new PointF(
                (float)
                    ThreadLocalRandom.current()
                        .nextDouble(offset * 2, scoringZone.getAllowedValues().right - offset),
                (float)
                    ThreadLocalRandom.current()
                        .nextDouble(offset, scoringZone.getAllowedValues().bottom - offset)));
      }
      return true;
    }
    return false;
  ...
  public static boolean isInRange(ScoringZone scoringZone, PlayerBall playerBall) {
    return scoringZone.getRadius()
        > (sqrt(
                pow(playerBall.getCenterX() - scoringZone.getCenterX(), 2)
                    + pow(playerBall.getCenterY() - scoringZone.getCenterY(), 2))
            + playerBall.getRadius());
  }
