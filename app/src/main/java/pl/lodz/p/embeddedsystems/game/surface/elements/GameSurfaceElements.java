...
public class GameSurfaceElements {
  ...
  public GameSurfaceElements(@NonNull Context context) {
    gameSurfaceViewModel =
        new ViewModelProvider((ViewModelStoreOwner) context).get(GameSurfaceViewModel.class);
  ...
  /**
   * Ustawienie obserwatora położenia pozycji gracza. Przelicza punkty.
   *
   * @param context kontekst aplikacji (obiekt aktywności)
   */
            ...
            orientatedData -> {
              if (gameSurfaceViewModel.getNonNullValueOf(gameSurfaceViewModel.getIsStarted())) {
                ...
                if (GameSurfaceConditions.checkRules(
                  ...
                  int newPoints =
                      (int)
                          (GameSurfaceConditions.getPointsInc()
                              * Math.max(1, steps - significantMotion));
              ...
              if (rotation == Surface.ROTATION_0 && lastKnownRotation != Surface.ROTATION_0) {
                if (lastKnownRotation == Surface.ROTATION_90) {
                  getScoringZone()
                      .setCenter(
                          new PointF(
                              displayMetrics.heightPixels - getScoringZone().getCenterY(),
                              getScoringZone().getCenterX()));
                  getPlayer()
                      .setCenter(
                          new PointF(
                              displayMetrics.heightPixels - getPlayer().getCenterY(),
                              getPlayer().getCenterX()));
                  swapAllowedValues();
                } else if (lastKnownRotation == Surface.ROTATION_270) {
                  getScoringZone()
                      .setCenter(
                          new PointF(
                              getScoringZone().getCenterY(),
                              displayMetrics.widthPixels - getScoringZone().getCenterX()));
                  getPlayer()
                      .setCenter(
                          new PointF(
                              getPlayer().getCenterY(),
                              displayMetrics.widthPixels - getPlayer().getCenterX()));
                  swapAllowedValues();
                }

                //                            } else if (rotation == Surface.ROTATION_180 &&
                // lastKnownRotation != Surface.ROTATION_180) {
                //
              } else if (rotation == Surface.ROTATION_90
                  && lastKnownRotation != Surface.ROTATION_90) {
                if (lastKnownRotation == Surface.ROTATION_0) {
                  getScoringZone()
                      .setCenter(
                          new PointF(
                  ...
                  && lastKnownRotation != Surface.ROTATION_270) {
                if (lastKnownRotation == Surface.ROTATION_0) {
                  getScoringZone()
                      .setCenter(
                          new PointF(
  ...
  private void swapAllowedValues() {
    if (playerBall.getAllowedValues().right == displayMetrics.heightPixels
        && playerBall.getAllowedValues().bottom == displayMetrics.widthPixels) {
  ...
  public void createPlayer(float x, float y, float radius) {
    playerBall =
        ElementsCreator.createBall(
            x, y, CreatorUtils.getPaint(Paint.Style.FILL, Color.RED), radius);
  }
  ...
  public void createScoringZone(float x, float y, float radius) {
    scoringZone =
        ElementsCreator.createScoringZone(
            x, y, CreatorUtils.getPaint(Paint.Style.FILL, Color.YELLOW), radius);
  }
  ...
  public void onTouchEvent() {
    if (this.gameSurfaceViewModel.getNonNullValueOf(
      ...
      this.gameSurfaceViewModel
          ...
          .setValue(
              this.gameSurfaceViewModel.getNonNullValueOf(
                      this.gameSurfaceViewModel.getGainedScore())
                  + ThreadLocalRandom.current().nextInt(80)
                  + 20);
    }
  }
}
